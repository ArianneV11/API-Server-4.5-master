...
module.exports =
    class AccountsController extends require('./Controller') {
        constructor(HttpContext,) {
            super(HttpContext, new usersRepository(),false, true /* read authorisation */);
        ...
        remove(id) { // warning! this is not an API endpoint
            //Effacer les images de l'utilisateur.
            
            let image = new ImagesRepository();
            let images = image.getAll("");
            //let exist = false;
            for(let image of images) {
                if(image.UserId == id){
                    
                    //super.image.remove(image.UserId)
                }
            }
            super.remove(id);
